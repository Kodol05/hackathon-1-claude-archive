...
## 3단계 (마감 16:40까지) — 파일 단위로 갈라 충돌을 막는다
- 김현민: **문서만** 담당 (`README.md`, `CLAUDE.md`, `docs/`). 코드 파일은 건드리지 않는다
- 이정호: **코드만** 담당 (`index.html`, `app.js`, `style.css`의 일정 구역). 일정 → 활동 전환
- 박재웅: 보고서 (`tools/extract_prompts.py`로 프롬프트 추출해 3장 작성)
